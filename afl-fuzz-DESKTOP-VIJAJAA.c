...
static struct loghistory *loghead, *lognow, *tmploghead, *tmplognow; 
...
static struct start_byte *dict2d, *dict2d_cur, *dict2d_prev;
static struct change_byte *dict1d, *dict1d_cur, *tmpdict1d_cur, *tmpdict1d;
...
#define STAGE_OverWriteExtra 17
#define STAGE_InsertExtra 18
...
#define FLIP_BIT(_ar, _b) do { \
	...
	for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
        ...
        if(tmploghead == NULL)
		{
			tmploghead = (struct loghistory*) malloc(sizeof(struct loghistory));
		...
		{
			struct loghistory *tmptmplog = (struct loghistory*) malloc(sizeof(struct loghistory));
			tmptmplog->indata = out_buf[stage_cur_byte];
			tmptmplog->bytelen = 1;
			tmptmplog->type = 0;
			tmptmplog->next = NULL;
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l) - 1) - EFF_APOS(_p) + 1)
	...
	for (i = 0; i < len - 1; i++) {
		...
		{
			struct loghistory *tmptmplog = (struct loghistory*) malloc(sizeof(struct loghistory));
			tmptmplog->indata = *(u16*)(out_buf + stage_cur_byte); //(u32)(out_buf[stage_cur_byte]) << 8 + (u32)out_buf[stage_cur_byte + 1];
			tmptmplog->bytelen = 2;
			tmptmplog->type = 0; //overwrite
			tmptmplog->next = NULL;
			tmplognow->next = tmptmplog;
	...
	for (i = 0; i < len - 3; i++) {
		...
		{
			struct loghistory *tmptmplog = (struct loghistory*) malloc(sizeof(struct loghistory));
			tmptmplog->indata = *(u32*)(out_buf + stage_cur_byte); // (out_buf[stage_cur_byte]) << 24 + out_buf[stage_cur_byte + 1] << 16 + out_buf[stage_cur_byte + 2] << 8 + out_buf[stage_cur_byte + 3];
			tmptmplog->bytelen = 4;
			tmptmplog->type = 0; //overwrite
			tmptmplog->next = NULL;
			tmplognow->next = tmptmplog;
...
skip_bitflip:
	...
	for (i = 0; i < len - 1; i++) {
		...
		{
			struct loghistory *tmptmplog = (struct loghistory*) malloc(sizeof(struct loghistory));
			tmptmplog->indata = *(u16*)(out_buf + i); //(u32)(out_buf[stage_cur_byte]) << 8 + (u32)out_buf[stage_cur_byte + 1];
			tmptmplog->bytelen = 2;
			tmptmplog->type = 0; //overwrite
			tmptmplog->next = NULL;
			tmplognow->next = tmptmplog;
	...
	for (i = 0; i < len - 3; i++) {
		...
		{
			struct loghistory *tmptmplog = (struct loghistory*) malloc(sizeof(struct loghistory));
			tmptmplog->indata = *(u32*)(out_buf + i); //(u32)(out_buf[stage_cur_byte]) << 8 + (u32)out_buf[stage_cur_byte + 1];
			tmptmplog->bytelen = 4;
			tmptmplog->type = 0; //overwrite
			tmptmplog->next = NULL;
			tmplognow->next = tmptmplog;
...
skip_arith:
	...
	for (i = 0; i < len; i++) {
		...
		{
			struct loghistory *tmptmplog = (struct loghistory*) malloc(sizeof(struct loghistory));
			tmptmplog->indata = (out_buf[i]); //(u32)(out_buf[stage_cur_byte]) << 8 + (u32)out_buf[stage_cur_byte + 1];
			tmptmplog->bytelen = 1;
			tmptmplog->type = 0; //overwrite
			tmptmplog->next = NULL;
			tmplognow->next = tmptmplog;
		...
		for (j = 0; j < extras_cnt; j++) {
			...
			switch (last_len) {
				...
				case 2:{
					...
					{
						struct loghistory *tmptmplog = (struct loghistory*) malloc(sizeof(struct loghistory));
						tmptmplog->indata = *(u16*)(out_buf + i);
						tmptmplog->bytelen = 2;
						tmptmplog->type = 0; //overwrite
						tmptmplog->next = NULL;
						tmplognow->next = tmptmplog;
				...
				case 4:{
					...
					{
						struct loghistory *tmptmplog = (struct loghistory*) malloc(sizeof(struct loghistory));
						tmptmplog->indata = *(u32*)(out_buf + i); 
						tmptmplog->bytelen = 4;
						tmptmplog->type = 0; //overwrite
						tmptmplog->next = NULL;
						tmplognow->next = tmptmplog;
...
//herehere
{
	...
	for(tmpjj = 0; tmpjj < search_count; tmpjj++)
	{
		...
		{
		case 1:{
			tmpindata = out_buf[tmpii];
			break;}
		case 2:{
			tmpindata = *(u16*)(out_buf + tmpii);
			break;}
		case 4:{
			tmpindata = *(u32*)(out_buf + tmpii);
			break;}
		...
		while(!feof(fp))
		{
			...
			case 0:{
				...
				u64 tmpsubdata_count;
					fscanf(fp,"%u",&tmpindata);    //indata
					fscanf(fp,"%d",&tmpbytelen);    //bytelen
					fscanf(fp,"%llu",&tmptotalcount);    //totalcount
					fscanf(fp,"%llu",&tmpsubdata_count);    //subdata_count
						...
						if(dict2d_hash[tmpindata % 4096])
						{
							...
							if(dict2d_cur ==NULL)
							{
								dict2d_cur = (struct start_byte*) malloc(sizeof(struct start_byte));
								...
								fscanf(fp,"%s",dicttxt);
								if(strcmp(dicttxt, "startbyte\n") != 0 )
								{
									FATAL("error startbyte");
								}
								state_dict = 1;
							}
							else
			...
			case 1:{
				...
				u64 tmpcountnum;
				double tmpprob;
				fscanf(fp,"%u",&tmpoutdata);
				fscanf(fp,"%d",&tmptype);
				fscanf(fp,"%llu",&tmpcountnum);
				fscanf(fp,"%lf",&tmpprob);
				...
				if(dict1d_cur)
					...
					dict1d_cur = (struct change_byte*) malloc(sizeof(struct change_byte));
