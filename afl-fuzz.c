...
int select_algorithm(int extras)
{
  ...
  if (start_distill == 0)
    operator_number = operator_number - 3;
  else if (extras < 2)
...
EXP_ST void setup_shm(void)
{
  ...
  if (!in_bitmap)
    memset(virgin_bits, 255, afl_map_size);
...
static void show_stats(void)
{
  ...
  if (limit_time_sig == 1)
  {
    if (key_puppet == 0)
    {
      if (key_module == 0)
      {
        sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "EMS (pilot_fuzzing)", use_banner);
      }
      else if (key_module == 1)
      {
        sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "EMS (core_fuzzing)", use_banner);
      }
      else if (key_module == 2)
      {
        sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "EMS (pso_updating)", use_banner);
      }
    }
    else
    {
      ...
      if (key_module == 0)
      {
        sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "EMS + pacemaker (pilot_fuzzing)", use_banner);
      }
      else if (key_module == 1)
      {
        sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "EMS + pacemaker (core_fuzzing)", use_banner);
      }
      else if (key_module == 2)
      {
        sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "EMS + pacemaker (pso_updating)", use_banner);
      }
    }
  }
  else
  {
    sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "AFL", use_banner);
  }
...
#define SP20 SP10 SP10
  ...
  sprintf(tmp, "%0.02f%% / %0.02f%%", ((double)queue_cur->bitmap_size) * 100 / afl_map_size, t_byte_ratio);
